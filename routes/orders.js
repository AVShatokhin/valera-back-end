...
async function ORDERS_create(connection, ans, req) {
  return new Promise((resolve) => {
    connection.query(
      "insert into orders set admin_id=?, client_id=?, worker_id=?, order_number=?, order_works=?, summ=?, carName=?, carNum=?, ts_create=now();",
      [
        req.body.admin_id,
        req.body.client_id,
        req.body.worker_id,
...
async function ORDERS_change(connection, ans, req) {
  return new Promise((resolve) => {
    connection.query(
      "update orders set admin_id=?, client_id=?, worker_id=?, order_number=?, order_works=?, summ=?, carName=?, carNum=? where order_id=?",
      [
        req.body.admin_id,
        req.body.client_id,
        req.body.worker_id,
...
async function ORDERS_get_by_ts(connection, ans, lts) {
  return new Promise((resolve) => {
    connection.query(
      "select unix_timestamp(lts) as last_ts, order_id, worker_id, client_id, admin_id, order_number, summ, payed, closed, order_works, carName, carNum, " +
...
async function ORDERS_close_smena(connection, ans) {
  return new Promise((resolve) => {
    ...
    connection.query(
      "select client_id, admin_id, worker_id, order_number, summ, payed, closed, order_works, carName, carNum, " +
      ...
      (err, res) => {
        ...
        res.forEach((i) => {
          smena.push(
            `(${i.client_id}, ${i.admin_id}, ${i.worker_id}, ${i.order_number}, ${i.summ}, '${i.payed}', '${i.closed}', '${i.order_works}', '${i.carName}', '${i.carNum}', FROM_UNIXTIME(${i.lts}), FROM_UNIXTIME(${i.ts_create}), FROM_UNIXTIME(${i.ts_close}), FROM_UNIXTIME(${i.ts_pay}))`
          );
        });

        // console.log(smena);
        connection.query(
          `insert into orders_stat (client_id, admin_id, worker_id, order_number, summ, payed, closed, order_works, carName, carNum, lts, ts_create, ts_close, ts_pay) values ${smena.join(
            ","
          )};`,
          [],
          (err, res) => {
